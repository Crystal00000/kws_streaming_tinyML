...
from kws_streaming.train_tinyml import base_parser
from kws_streaming.train_tinyml import train
import kws_streaming.train_tinyml.test as test
...
def main(_):
  ...
  flags.model_name = 'ds_cnn_tinyml'
  ...
  if flags.train:
    ...
    # Model training
    best_model_num = train.train(flags)
  ...
  test.convert_model_saved(flags, 'non_stream', 
                           modes.Modes.NON_STREAM_INFERENCE, best_model_num = best_model_num)
  try:
    test.convert_model_saved(flags, 'stream_state_internal',  
                             modes.Modes.STREAM_INTERNAL_STATE_INFERENCE, best_model_num = best_model_num)
  ...
  folder_name = 'tf'
  if True:
    test.tf_non_stream_model_accuracy(flags, best_model_num, folder_name)
  # with TF.
  ...
  for opt_name, optimizations in name2opt.items():
    ...
    test.convert_model_tflite(flags, best_model_num, folder_name, mode, file_name,
                              optimizations = optimizations)
    
    test.tflite_non_stream_model_accuracy(flags, folder_name, file_name)
    
    model_is_streamable = True # maybe not
    
    # set input data shape for testing inference in streaming mode
    flags.data_shape = modes.get_tinyml_input_shape( # (360, 1, 1)
    ...
    if model_is_streamable:
      ...
      if not opt_name: 
        ...
        if True:
          try:
            ...
            test.tf_stream_state_internal_model_accuracy(flags, best_model_num, folder_name)
          except (ValueError, IndexError) as e:
            logging.info('FAILED to run TF streaming: %s', e)
      ...
      try:
        ...
        test.convert_model_tflite(flags, best_model_num, folder_name, mode, file_name,
                                  optimizations=optimizations)
        test.convert_model_tflite(flags, best_model_num, opt_name + 'tflite_stream_state_internal', modes.Modes.STREAM_INTERNAL_STATE_INFERENCE, 'stream_state_internal.tflite',
                                  optimizations=optimizations)
