...
def convert_to_inference_model(model, input_tensors, mode): # IMPORTANT
  ...
  with tf.name_scope(scope_name):
    ...
    # input_tensors =  [<tf.Tensor 'IEGM_input:0' shape=(1, 1, 10, 1) dtype=float32>]
...
def to_streaming_inference(model_non_stream, flags, mode): # important
  print("***enter to_streaming_inference")
  ...
  input_data_shape = modes.get_tinyml_input_shape(flags, mode)
  ...
  # model_non_stream.input =  Tensor("input_1:0", shape=(1, 1, 1250, 1), dtype=float32)
  # model_non_stream.input.shape =  (1, 1, 1250, 1)
  # dtype = <dtype: 'float32'>
  input_tensors = [
      tf.keras.layers.Input(
          shape = input_data_shape, batch_size = 1, dtype = dtype, name = 'IEGM_input')
  ]
  print("***input_tensors = ", input_tensors)
  
  if (isinstance(model_non_stream.input, (tuple, list)) and len(model_non_stream.input) > 1): # doesn't enter
  ...
  print("***quantize_stream_scope = ", quantize_stream_scope)
...
def model_to_saved(model_non_stream,
  ...
  model.save(os.path.join(flags.train_dir, save_model_path, 'stream_internal.tf'), include_optimizer=False, save_format='tf')
  model.save(os.path.join(flags.train_dir, save_model_path, 'stream_internal.h5'), include_optimizer=False, save_format='h5')
